...
#include "sdLib.h"
#include "test_trigger_table.h" /* User defined Trigger Tables */
...
/*	int evt_Type = tiDecodeTriggerType(dma_dabufp, dCnt, 9);
	if(evt_Type != -1)
	{
		printf("evt_Type = %d", evt_Type);
	}
*/
/*
	unsigned int evT[BLOCKLEVEL] = {0};
	tiDecodeTriggerTypes(dma_dabufp, dCnt, 	BUFFERLEVEL, evT);
	int i;
...
/*
   ...
   printf("\nTRIGGER %d RECEIVED\n",counter);
   counter++;
   printf("Current Readout Count: %d\n",tiGetIntCount());
   int dCnt;

//Set TI output 1 high for diagnostics 
tiSetOutputPort(1,0,0,0);
//tiSetTriggerLock(1);
//printf("Trigger Locked");

// Readout the trigger block from the TI
//  Trigger Block MUST be readout first 
dCnt = tiReadTriggerBlock(dma_dabufp);
//dCnt = tiReadBlock(dma_dabufp, 5*BLOCKLEVEL+3+4,1);
//tiDecodeTriggerType(dma_dabufp, dCnt, 33);
printf("dCnt =  %d", dCnt);
if(dCnt<=0)
{
printf("No TI Trigger data or error.  dCnt = %d\n",dCnt);
...
unsigned int trigType[BUFFERLEVEL]; 
int i;
for(i = 0; i < BUFFERLEVEL; i++){
trigType[i] = 0;	
}
tiDecodeTriggerTypes(dma_dabufp, dCnt, BUFFERLEVEL, trigType);
for(i =0; i < BUFFERLEVEL; i++)
printf("TrigType[%d] = %d\n", i, trigType[i]);

//int evt_Type = tiDecodeTriggerType(dma_dabufp, dCnt, 33);
//printf("evt_Type = %d", evt_Type);
